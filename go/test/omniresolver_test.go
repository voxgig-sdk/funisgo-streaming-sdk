...
import (
	...
	omni "github.com/voxgig-sdk/funisgo-streaming-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FunisgoStreamingError); is {
				out := map[string]any{
					"name":    "FunisgoStreamingError",
...
func omniCtx(arg any, client *sdk.FunisgoStreamingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FunisgoStreamingSDK); is {
			client = live
