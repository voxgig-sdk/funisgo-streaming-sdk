...
func TestMovieDirect(t *testing.T) {
	t.Run("direct-list-movie", func(t *testing.T) {
		setup := movieDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-movie", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-movie", func(t *testing.T) {
		setup := movieDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-movie", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type movieDirectSetupResult struct {
	client *sdk.FunisgoStreamingSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func movieDirectSetup(mockres any) *movieDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"FUNISGO_STREAMING_TEST_MOVIE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["FUNISGO_STREAMING_TEST_MOVIE_ENTID"]; ok {
		...
		return &movieDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &movieDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
