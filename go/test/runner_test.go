...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FUNISGO_STREAMING_TEST_LIVE") == "TRUE" ||
		os.Getenv("FUNISGO_STREAMING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FUNISGO_STREAMING_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FunisgoStreamingSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FunisgoStreamingError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FunisgoStreamingSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FunisgoStreamingError{Msg: msg, Code: code}
}
