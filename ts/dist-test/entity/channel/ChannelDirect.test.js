...
(0, node_test_1.describe)('ChannelDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FunisgoStreamingSDK({
    ...
    (0, node_test_1.test)('direct-load-channel', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-channel', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-channel', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-channel', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.FUNISGO_STREAMING_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FunisgoStreamingSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.FUNISGO_STREAMING_APIKEY,
        }));
        let idmap = env['FUNISGO_STREAMING_TEST_CHANNEL_ENTID'];
    ...
    const client = new __1.FunisgoStreamingSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ChannelDirect.test.js.map