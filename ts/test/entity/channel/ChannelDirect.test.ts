...
describe('ChannelDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new FunisgoStreamingSDK({
  ...
  test('direct-load-channel', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-channel', setup.live)) return
  ...
  test('direct-list-channel', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-channel', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FUNISGO_STREAMING_TEST_CHANNEL_ENTID']
