...
//     (\`new FunisgoStreamingSDK(...)\` / \`FunisgoStreamingSDK.test(...)\` become
//     \`FunisgoStreamingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FunisgoStreamingSDK'
...
const TEST_SEED = {"entity":{"channel":{"test01":{"id":"test01"}},"movie":{"test01":{"id":"test01"}},"series":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FunisgoStreamingSDK(...)\` and any
// \`FunisgoStreamingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FunisgoStreamingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FunisgoStreamingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
