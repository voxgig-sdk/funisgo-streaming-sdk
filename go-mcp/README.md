...
[MCP](https://modelcontextprotocol.io) server exposing the FunisgoStreaming SDK as
two agent tools — `funisgo-streaming_list` and `funisgo-streaming_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/funisgo-streaming-mcp)
make build

# 2. Provide credentials via the environment
export FUNISGO_STREAMING_APIKEY=sk_live_xxx
...
```jsonc
// funisgo-streaming_list: first page of records
{ "entity": "channel" }
{ "entity": "channel", "query": { } }

// funisgo-streaming_load: one record by id
{ "entity": "channel", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/funisgo-streaming-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user funisgo-streaming \
     -- "$PWD"/dist/*/funisgo-streaming-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `funisgo-streaming_list` and `funisgo-streaming_load` tools now appear
   in new sessions. Ask the agent to *"list channel using funisgo-streaming"*
   and it calls `funisgo-streaming_list` with `{"entity":"channel"}`.
...
```sh
export FUNISGO_STREAMING_APIKEY=sk_live_xxx            # API key
export FUNISGO_STREAMING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `funisgo-streaming_list` tool
...
### Call the `funisgo-streaming_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `funisgo-streaming_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `funisgo-streaming_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FUNISGO_STREAMING_APIKEY` | API key sent with every request. |
| `FUNISGO_STREAMING_BASE` | Optional override of the API base URL. |
