...
class FunisgoStreamingSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FunisgoStreamingHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FunisgoStreamingHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FunisgoStreamingFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FunisgoStreamingFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FunisgoStreamingUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FunisgoStreamingHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FunisgoStreamingHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FunisgoStreamingHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FunisgoStreamingError($op . "_allow",
                "FunisgoStreamingSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FunisgoStreamingHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FunisgoStreamingError("graphql_error",
                "FunisgoStreamingSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Movie($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_movie === null) {
                $this->_movie = new MovieEntity($this, null);
            }
            return $this->_movie;
        }
        return new MovieEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new FunisgoStreamingSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
