...
class MovieEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = FunisgoStreamingSDK::test(null, null);
        $ent = $testsdk->Movie(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Movie(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Movie(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = movie_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "movie." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $movie_ref01_ent = $client->Movie(null);
        ...
        $movie_ref01_list_result = $movie_ref01_ent->list($movie_ref01_match, null);
        $this->assertIsArray($movie_ref01_list_result);
        ...
        $movie_ref01_ent->remove($movie_ref01_match_rm0, null);

        // LIST
        $movie_ref01_match_rt0 = [];

        $movie_ref01_list_rt0_result = $movie_ref01_ent->list($movie_ref01_match_rt0, null);
        $this->assertIsArray($movie_ref01_list_rt0_result);
...
function movie_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/movie/MovieTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["movie01", "movie02", "movie03"] as $k) {
