...
class ChannelEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FunisgoStreamingSDK::test($seed, null);
        $seen = iterator_to_array($base->Channel(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FunisgoStreamingConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FunisgoStreamingSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Channel(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FUNISGO_STREAMING_TEST_CHANNEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $channel_ref01_data = Helpers::to_map(is_object($channel_ref01_data_result) && method_exists($channel_ref01_data_result, 'data_get') ? $channel_ref01_data_result->data_get() : $channel_ref01_data_result);
        ...
        $channel_ref01_resdata_up0 = Helpers::to_map(is_object($channel_ref01_resdata_up0_result) && method_exists($channel_ref01_resdata_up0_result, 'data_get') ? $channel_ref01_resdata_up0_result->data_get() : $channel_ref01_resdata_up0_result);
        ...
        $channel_ref01_data_dt0_load_result = Helpers::to_map(is_object($channel_ref01_data_dt0_loaded) && method_exists($channel_ref01_data_dt0_loaded, 'data_get') ? $channel_ref01_data_dt0_loaded->data_get() : $channel_ref01_data_dt0_loaded);
...
function channel_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FUNISGO_STREAMING_TEST_CHANNEL_ENTID");
