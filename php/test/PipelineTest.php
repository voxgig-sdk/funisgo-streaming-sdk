...
// FunisgoStreaming SDK pipeline test
//
...
{
    private static function utility(): FunisgoStreamingUtility
    {
        return new FunisgoStreamingUtility();
    }
    ...
    private static function ctx(array $over = []): FunisgoStreamingContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FunisgoStreamingContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FunisgoStreamingOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FunisgoStreamingError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FunisgoStreamingOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FunisgoStreamingMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FunisgoStreamingOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FunisgoStreamingMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FunisgoStreamingOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FunisgoStreamingMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FunisgoStreamingSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FunisgoStreamingMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FunisgoStreamingResponse([]),
            'result' => new FunisgoStreamingResult([]),
        ]);
        [, $err] = FunisgoStreamingMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec([]),
            'response' => null,
            'result' => new FunisgoStreamingResult([]),
        ]);
        [, $err] = FunisgoStreamingMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec(['step' => 's']),
            'response' => new FunisgoStreamingResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FunisgoStreamingResult([]),
        ]);
        [, $err] = FunisgoStreamingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec(['step' => 's']),
            'response' => new FunisgoStreamingResponse(self::resp(200, ['v' => 1])),
            'result' => new FunisgoStreamingResult([]),
        ]);
        [, $err] = FunisgoStreamingMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec(['step' => 's']),
            'response' => new FunisgoStreamingResponse(self::resp(200, ['v' => 2])),
            'result' => new FunisgoStreamingResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FunisgoStreamingMakeResponse::call($ctx);
    ...
    {
        $preset = new FunisgoStreamingResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec([]),
            'response' => new FunisgoStreamingResponse([]),
            'result' => new FunisgoStreamingResult([]),
        ...
        [$got, $err] = FunisgoStreamingMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FunisgoStreamingResult([])]);
        [, $err] = FunisgoStreamingMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FunisgoStreamingSpec([]), 'result' => null]);
        [, $err] = FunisgoStreamingMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FunisgoStreamingSpec(['step' => 's']),
            'result' => new FunisgoStreamingResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FunisgoStreamingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FunisgoStreamingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FunisgoStreamingSpec(['step' => 's']),
            'result' => new FunisgoStreamingResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FunisgoStreamingOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FunisgoStreamingMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FunisgoStreamingResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FunisgoStreamingSpec([]),
            'result' => new FunisgoStreamingResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FunisgoStreamingMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FunisgoStreamingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FunisgoStreamingError('boom', 'boom');
        $utility->fetcher = function (FunisgoStreamingContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FunisgoStreamingSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FunisgoStreamingMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FunisgoStreamingContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FunisgoStreamingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FunisgoStreamingResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FunisgoStreamingMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FunisgoStreamingContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FunisgoStreamingSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FunisgoStreamingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FunisgoStreamingResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FunisgoStreamingSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FunisgoStreamingMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FunisgoStreamingMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FunisgoStreamingMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FunisgoStreamingResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FunisgoStreamingDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FunisgoStreamingResult(['ok' => false])]);
        $this->expectException(FunisgoStreamingError::class);
        FunisgoStreamingDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FunisgoStreamingResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FunisgoStreamingMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FunisgoStreamingMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FunisgoStreamingBaseFeature();
        $b = new FunisgoStreamingBaseFeature();
        FunisgoStreamingFeatureAdd::call($ctx, $a);
        FunisgoStreamingFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FunisgoStreamingBaseFeature
    {
        $f = new FunisgoStreamingBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FunisgoStreamingFeatureAdd::call($ctx, self::named_feature('a'));
        FunisgoStreamingFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        FunisgoStreamingFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        FunisgoStreamingFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        FunisgoStreamingFeatureAdd::call($ctx, $replace);
        ...
        FunisgoStreamingFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FunisgoStreamingContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FunisgoStreamingSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FunisgoStreamingPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FunisgoStreamingPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FunisgoStreamingResponse(['headers' => null]),
            'result' => new FunisgoStreamingResult([]),
        ]);
        FunisgoStreamingResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        FunisgoStreamingResultBody::call($ctx);
