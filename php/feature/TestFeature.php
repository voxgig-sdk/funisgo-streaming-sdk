...
// FunisgoStreaming SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class FunisgoStreamingTestFeature extends FunisgoStreamingBaseFeature
{
    ...
    public function init(FunisgoStreamingContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (FunisgoStreamingContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = FunisgoStreamingParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (FunisgoStreamingContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(FunisgoStreamingContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = FunisgoStreamingParam::call($ctx, $k);
