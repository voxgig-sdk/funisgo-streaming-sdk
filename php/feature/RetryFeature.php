...
// FunisgoStreaming SDK retry feature
...
class FunisgoStreamingRetryFeature extends FunisgoStreamingBaseFeature
{
    ...
    public function init(FunisgoStreamingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FunisgoStreamingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FunisgoStreamingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FunisgoStreamingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
