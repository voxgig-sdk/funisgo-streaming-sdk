...
class FunisgoStreamingMakeError
{
    public static function call(?FunisgoStreamingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FunisgoStreamingContext([], null);
        }
        $op = $ctx->op ?? new FunisgoStreamingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FunisgoStreamingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FunisgoStreamingError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FunisgoStreamingError('', $msg, $ctx);
        ...
        if ($err instanceof FunisgoStreamingError) {
