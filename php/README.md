...
print_r(array_map(fn($item) => $item->data_get(), $series));
```
...
```php
$client = FunisgoStreamingSDK::test($testopts, $sdkopts);
```
...
```
php/
├── funisgostreaming_sdk.php          -- Main SDK class
...
The main class (`funisgostreaming_sdk.php`) exports the SDK class
