...
AQL-driven command-line client **and** interactive REPL for the FunisgoStreaming
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/funisgo-streaming-cli)
make build

# 2. See usage (words, entities, env vars)
./funisgo-streaming-cli --help
...
./funisgo-streaming-cli load 1 channel            # {id:1} shorthand
./funisgo-streaming-cli load '{id:1}' channel       # explicit match map
./funisgo-streaming-cli update '{name:"x"}' channel
./funisgo-streaming-cli list movie

# 5. Override the API base URL for a single call
FUNISGO_STREAMING_BASE=https://api.example.com ./funisgo-streaming-cli list channel

# 6. No arguments -> interactive REPL
./funisgo-streaming-cli
funisgo-streaming> list channel
funisgo-streaming> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/funisgo-streaming-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/funisgo-streaming-cli list channel
   ```
...
```sh
./funisgo-streaming-cli load 1 channel          # scalar shorthand for {id:1}
./funisgo-streaming-cli load '{id:1}' channel     # explicit match map
```
...
```sh
./funisgo-streaming-cli update '{id:1,name:"new"}' channel
```
...
Run with no arguments to open a REPL (prompt `funisgo-streaming>`). Each line is
evaluated as its own AQL expression:

```text
$ ./funisgo-streaming-cli
funisgo-streaming> list channel
funisgo-streaming> /help
funisgo-streaming> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `channel`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/funisgo-streaming-cli`. |
...
(`./funisgo-streaming-cli <expr>`) and interactively (the REPL), and expressions compose the
