// Command FunisgoStreaming-cli is a CLI + REPL for the FunisgoStreaming SDK,
...
const prompt = "funisgo-streaming"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "channel movie series"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FUNISGO_STREAMING_APIKEY (api key), FUNISGO_STREAMING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FUNISGO_STREAMING_APIKEY carries the API key and
	// FUNISGO_STREAMING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FUNISGO_STREAMING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FUNISGO_STREAMING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFunisgoStreamingSDK(opts)
