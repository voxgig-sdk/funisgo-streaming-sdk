...
require("funisgo-streaming_types")
...
local FunisgoStreamingSDK = {}
FunisgoStreamingSDK.__index = FunisgoStreamingSDK
...
FunisgoStreamingSDK._make_feature = _make_feature


function FunisgoStreamingSDK.new(options)
  local self = setmetatable({}, FunisgoStreamingSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FunisgoStreamingSDK:options_map()
...
function FunisgoStreamingSDK:get_utility()
  return Utility.copy(self._utility)
end


function FunisgoStreamingSDK:get_root_ctx()
  return self._rootctx
end


function FunisgoStreamingSDK:prepare(fetchargs)
...
function FunisgoStreamingSDK:direct(fetchargs)
...
function FunisgoStreamingSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function FunisgoStreamingSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "FunisgoStreamingSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function FunisgoStreamingSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FunisgoStreamingSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FunisgoStreamingSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function FunisgoStreamingSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FunisgoStreamingSDK.new(sdkopts)
  sdk.mode = "test"
