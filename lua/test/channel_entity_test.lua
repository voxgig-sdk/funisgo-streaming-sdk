...
describe("ChannelEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Channel(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Channel(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    -- LIST
    local channel_ref01_match = {}

    local channel_ref01_list_result, err = channel_ref01_ent:list(channel_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(channel_ref01_list_result)
    ...
    -- UPDATE
    local channel_ref01_data_up0_up = {
      id = channel_ref01_data["id"],
    }

    local channel_ref01_markdef_up0_name = "category"
    local channel_ref01_markdef_up0_value = "Mark01-channel_ref01_" .. tostring(setup.now)
    ...
    -- LIST
    local channel_ref01_match_rt0 = {}

    local channel_ref01_list_rt0_result, err = channel_ref01_ent:list(channel_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(channel_ref01_list_rt0_result)
