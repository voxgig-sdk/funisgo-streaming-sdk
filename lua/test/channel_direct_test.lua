...
describe("ChannelDirect", function()
  it("should direct-list-channel", function()
    local setup = channel_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-channel", setup.live and "live" or "unit")
  ...
  it("should direct-load-channel", function()
    local setup = channel_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-channel", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function channel_direct_setup(mockres)
