...
describe("MovieDirect", function()
  it("should direct-list-movie", function()
    local setup = movie_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-movie", setup.live and "live" or "unit")
  ...
  it("should direct-load-movie", function()
    local setup = movie_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-movie", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function movie_direct_setup(mockres)
