...
describe("MovieEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FUNISGOSTREAMING_TEST_MOVIE_ENTID JSON to run live")
      return
    end
    ...
    local movie_ref01_markdef_up0_name = "created_at"
    ...
    assert.is_nil(err)
    local movie_ref01_resdata_up0 = helpers.to_map(movie_ref01_resdata_up0_result)
    ...
    assert.is_nil(err)
    local movie_ref01_data_dt0_load_result = helpers.to_map(movie_ref01_data_dt0_loaded)
...
function movie_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FUNISGOSTREAMING_TEST_MOVIE_ENTID")
