...
local MovieEntity = {}
MovieEntity.__index = MovieEntity


function MovieEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MovieEntity)
  self._name = "movie"
  self._client = client
...
function MovieEntity:get_name()
  return self._name
end


function MovieEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MovieEntity.new(self._client, opts)
end
...
function MovieEntity:mark_deleted()
  self._deleted = true
end


function MovieEntity:deleted()
  return true == self._deleted
end


function MovieEntity:data_set(args)
  if args ~= nil then
...
function MovieEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MovieEntity:match_set(args)
  if args ~= nil then
...
function MovieEntity:match_get()
...
function MovieEntity:stream(action, args, callopts)
...
function MovieEntity:_run_op(ctx, post_done)
  local utility = self._utility
