...
// FunisgoStreaming SDK feature test
//
...
class FtCtrl extends FunisgoStreamingControl
{
...
class FtClient
{
    ...
    private FunisgoStreamingUtility $utility;

    public function __construct(FunisgoStreamingUtility $utility, array $options)
    {
    ...
    public function get_utility(): FunisgoStreamingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FunisgoStreamingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FunisgoStreamingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FunisgoStreamingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FunisgoStreamingConfig::make_feature($name);
    ...
    private static function buildUrl(FunisgoStreamingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FunisgoStreamingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FunisgoStreamingResponse($fetched);
        }
    ...
    private function fail(FunisgoStreamingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FunisgoStreamingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
