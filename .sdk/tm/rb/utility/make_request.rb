...
module FunisgoStreamingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FunisgoStreamingResponse.new({})
    ...
    if fetch_err
      ...
      response = FunisgoStreamingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FunisgoStreamingResponse.new(fetched)
    else
