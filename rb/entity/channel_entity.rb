...
class ChannelEntity
  ...
  def make
    opts = @_entopts.dup
    ChannelEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Channel.load works with no args).
  ...
  #
  # @param reqmatch [ChannelListMatch, Hash, nil] match filter (any subset of
  #   Channel fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
