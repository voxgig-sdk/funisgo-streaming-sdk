...
require_relative 'FunisgoStreaming_types'
...
class FunisgoStreamingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FunisgoStreamingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FunisgoStreamingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FunisgoStreamingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FunisgoStreamingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FunisgoStreamingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FunisgoStreamingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FunisgoStreamingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue FunisgoStreamingError => err
    ...
    if fetched.is_a?(Hash)
      status = FunisgoStreamingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FunisgoStreamingError.new(
        "graphql_error", "FunisgoStreamingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FunisgoStreamingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
