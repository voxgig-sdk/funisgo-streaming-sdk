...
class TestMovieEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Movie(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Movie(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _movie_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "movie." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FUNISGO_STREAMING_TEST_MOVIE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        movie_ref01_ent = client.Movie(None)
        movie_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.movie"), "movie_ref01"))

        movie_ref01_data = helpers.to_map(runner.entity_data(movie_ref01_ent.create(movie_ref01_data, None)))
        ...
        movie_ref01_resdata_up0 = helpers.to_map(runner.entity_data(movie_ref01_ent.update(movie_ref01_data_up0_up, None)))
...
def _movie_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/movie/MovieTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FUNISGO_STREAMING_TEST_MOVIE_ENTID"))
