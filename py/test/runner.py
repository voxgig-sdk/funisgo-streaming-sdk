# FunisgoStreaming SDK test runner
...
class FunisgoStreamingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FunisgoStreamingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FunisgoStreamingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FunisgoStreamingTestRunner.getenv("FUNISGO_STREAMING_TEST_LIVE")
        override = FunisgoStreamingTestRunner.getenv("FUNISGO_STREAMING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FunisgoStreamingTestRunner.getenv(key)
        ...
        explain = FunisgoStreamingTestRunner.getenv("FUNISGO_STREAMING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FunisgoStreamingTestRunner._test_control is not None:
            return FunisgoStreamingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FunisgoStreamingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FunisgoStreamingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FunisgoStreamingTestRunner.load_test_control()
...
def load_env_local():
    FunisgoStreamingTestRunner.load_env_local()


def env_override(m):
    return FunisgoStreamingTestRunner.env_override(m)


def entity_data(v):
    return FunisgoStreamingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FunisgoStreamingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FunisgoStreamingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FunisgoStreamingTestRunner.load_test_control()


def live_client_options():
    return FunisgoStreamingTestRunner.live_client_options()


def live_delay_ms():
    return FunisgoStreamingTestRunner.live_delay_ms()
